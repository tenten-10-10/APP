...
- **招待に生のicloudリンクを一切載せない（1.2.56）**: 1.2.55では案内メールの
  「うまくいかないとき」の貼り付け用に**生の `icloud.com/share` を残していた**。
  未導入者がそれを長押しではなく**タップすると icloud.com のサインインに
  収束**し得る（＝ユーザーが最も嫌がった「まちがっても収束」）。対策:
  `extractShareURL` がラッパー `t.l0l0.app/join?s=…` も受理して中の
  icloud URLに復元するようにし、メールの貼り付け用リンクもラッパーに統一。
  → **案内メールに出るリンクはすべて `t.l0l0.app/join`。タップしても
